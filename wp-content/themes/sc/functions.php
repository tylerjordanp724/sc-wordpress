<?php
/**
 * sabrinaclaudio functions and definitions
 *
...
function sc_setup() {
	/*
		...
		* If you're building a theme based on sabrinaclaudio, use a find and replace
 ...
 */
function sc_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'sc_content_width', 640 );
}
add_action( 'after_setup_theme', 'sc_content_width', 0 );
...
function sc_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'sc' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'sc' ),
 ...
 */
function sc_scripts() {
	// wp_enqueue_style( 'sc-style', get_stylesheet_uri(), array(), _S_VERSION );
	// wp_style_add_data( 'sc-style', 'rtl', 'replace' );

	// wp_enqueue_script( 'sc-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );

	wp_enqueue_style( 'bootstrap', 'https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css', array(), '5.1.3','all');

	wp_enqueue_style( 'styles', get_template_directory_uri().'/css/style.css',array(), '1.0.0','all' );
